...
get_results = function(fitC,fitM,fitF,fitI){
  var.statC = sum(vcov(fitC)[-1,-1])
  slopeHC =sum(summary(fitC)$coef[c('rntHbA1c','x2.star'),"Estimate"])
  ...
  resC = data.table(resM,Sex="Sex-Combined")
...
fit_function = function(yname,data,adj){
  ...
  fitC = lm(y ~ rntHbA1c + x2.star, data=analdat) #does this make sense?
...
for(i in 1:34){
  ...
  fit_BaseM = lm(y.resid ~ rntHbA1c + x2.star, data=resid.data, subset=Sex=="Male")
  fit_BaseF = lm(y.resid ~ rntHbA1c + x2.star, data=resid.data, subset=Sex=="Female")
  fit_BaseI = lm(y.resid ~ Sex*(rntHbA1c + x2.star), data=resid.data)
  fit_BaseC = lm(y.resid ~ rntHbA1c + x2.star, data=resid.data)
  res_Base = data.table(get_results(fitF=fit_BaseF,fitM=fit_BaseM,fitI=fit_BaseI),
                        adjustment="base")
  res_Base$roi = roii
...
res_High.F = res_High.F[order(res_High.F$beta),]
roi.level = res_High.F$roi
...
res_Base_Horizontal = merge(res_High.M,res_High.F,by='roi',sort=F)
...
res_High$Sex = factor(res_High$Sex, levels=c("Male","Female"))
...
res$Sex = factor(res$Sex,levels=rev(c("Male","Female")))
res$signif.FDR = ""
...
ggp1 = p + 
  scale_y_continuous(limits =c(-0.515,0.8), 
...
png(paste('Plots/HbA1cLevels_roi_base_',Sys.Date(),'.png',sep=''),
    width=(12*1.5),height=(12*1.5),units='in',res=300)
print(ggp1+theme(strip.text.y.left = element_text(angle = 0,size=24)))
dev.off()

#table: write out a table ---------------------------------------------------------------
profiles_to_write = subset(res_Base_Horizontal,select=c(roi,beta.x,beta.y)) %>% 
  dplyr::rename(beta.male=beta.x,beta.female=beta.y)
